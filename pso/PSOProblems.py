...
class BPSOKnapsackProblem(PSOProblem):
        ...
        def __init__(self):
            ...
            for i in range(generations):
                sc.updateSwarm(swarm)
                if swarm._bestPositionFitness != -1 and swarm._bestPositionFitness < fitness:
                    fitness = swarm._bestPositionFitness
                    idx = i
                print ("Generation", i+1,"\t-> BestPos:", swarm._bestPosition, "\tBestFitness:", swarm._bestPositionFitness)
...
class BPSOTSPProblem(PSOProblem):
    ...
    def __init__(self):        
        ...
        numOfCities = 4
        solution    = TSPSolutionModel(graph, numOfCities, "A")  # TSP solution model will contain the Best Path
        popSize     = 50            # total number of particles
        ...
        # Swarm Initialization
        swarm   = SwarmModel()
        sc      = SwarmController("tsp", solution)   # this creates a SwarmController object which contains a ParticleController and a Neighbourhood controller
        sc.initSwarm(swarm, topology, popSize, dimensions)
        ...
        print ("\n===================================================================")
        print ("Number of edges:\t", int(dimensions / 2), "\nNum of cities:\t\t", numOfCities)
        if swarm._bestPositionFitness != -1:
